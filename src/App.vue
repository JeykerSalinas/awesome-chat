...
const modes: ModeOption[] = [
  {
    ...
    description: 'Botón flotante que abre un chat emergente en la esquina inferior derecha.',
  },
  {
    value: 'modal',
    label: 'Modal',
...
const activeMode = ref<ModeOption['value']>('floating')
...
<template>
  <main class="app-layout">
    <header class="hero">
      ...
      <p>
        Selecciona una modalidad para previsualizar el componente de chat y simular una conversación
        con datos mockeados.
      </p>
    </header>

    <section class="controls">
      <label class="controls__label" for="chat-mode">Modo de chat</label>
      <select
        id="chat-mode"
        v-model="activeMode"
      ...
      <p class="controls__description" data-testid="mode-description">
        {{ activeDescription }}
      </p>
    </section>

    <section class="preview" data-testid="chat-preview">
      <ChatWidget
        :key="activeMode"
        :mode="activeMode"
        title="Lovely Chat"
    ...
    <section class="follow-ups">
      ...
      <ul>
        <li v-for="option in followUps" :key="option">{{ option }}</li>
      </ul>
    </section>
  </main>
</template>
